...
actor class Verifier() {
    ...
    private func isRegistered(p : Principal) : Bool {
        switch (studentProfileStore.get(p)) {
            ...
            case (?StudentProfile) return true;
        };
    };
    ...
    public shared ({ caller }) func addMyProfile(profile : StudentProfile) : async Result.Result<(), Text> {
        ...
        if (isRegistered(caller)) return #err("You are already registered (" #Principal.toText(caller) # ")");
    ...
    public shared query ({ caller }) func seeAProfile(p : Principal) : async Result.Result<StudentProfile, Text> {
        ...
        switch (xProfile) {
            case (null) return #err("There is no profile registered with the received account");
